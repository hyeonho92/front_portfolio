...
$(document).ready(function() {
  ...
  $('.btn1').css('background','url(images/bar2.png)'); //1번째 버튼 불켜!! 
...
//		}
	   ...
	   for(var i=1;i<=imageCount;i++){ // i=>1 2 3 4 5
		  $('.btn'+i).css('background','url(images/bar1.png)'); //모든 버튼에 불 다꺼!!
	   }
	   $('.btn'+cnt).css('background','url(images/bar2.png)');//자기 버튼만 불켜~
  ...
  $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
	...
	for(var i=1;i<=imageCount;i++){
		$('.btn'+i).css('background','url(images/bar1.png)'); //모든 버튼 불끈다
	}
    ...
    $('.btn'+cnt).css('background','url(images/bar2.png)'); //클릭한 버튼의 불켜
